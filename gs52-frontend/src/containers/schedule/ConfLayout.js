...
import { ConfAxios, FloorAxios } from "src/modules/schedule/conf";
...
const ConfLayout = () => {
  ...
  const { conf_list, floor_list, conf_modal1, conf_modal2 } = useSelector(
    (state) => {
      return {
  ...
  return (
    <div>
      <div style={contentStyle}>
        <h4 style={{ color: "white", marginTop: "5px" }}>회의실 예약</h4>
      </div>
      <br />
      ...
      <CCardBody>
        ...
        {conf_modal1 === true ? <ConfModal></ConfModal> : null}
        {conf_modal2 === true ? <ConfDetail></ConfDetail> : null}
      </CCardBody>
    </div>
  );
};
