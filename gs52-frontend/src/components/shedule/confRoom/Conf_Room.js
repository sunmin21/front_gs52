...
export function ConfRoom(props) {
  ...
  const handleDateClick = async (arg) => {
    ...
    await dispatch(
      modalEndTime(moment(arg.dateStr).add(30, "m").format("HH:mm"))
    );
  ...
  const data = conf_list.map((item) => {
    if (
      moment(item.conf_DATE).format("YYYY-MM-DD") + "T" + item.conf_END <
    ...
    return {
      ...
      start:
        moment(item.conf_DATE).format("YYYY-MM-DD") + "T" + item.conf_START,
